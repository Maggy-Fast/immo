...
try {
    ...
    foreach ($tables as $table) {
        ...
        if (str_contains($tableName, 'notification')) {
    ...
    if ($tableExists) {
        ...
        $stmt = $pdo->prepare("SELECT * FROM notifications_whatsapp WHERE telephone = :telephone ORDER BY created_at DESC LIMIT 1");
        $stmt->execute(['telephone' => '774958446']);
